...
def generate_token_dict(vocab):
    ...
    # Assign each item in vocab a unique integer starting from 0
...
def prepocess_input_sequence(
    ...
    for token in tokens:
        if token in spc_tokens:
            # If the token is a special token (BOS, EOS, POSITIVE, NEGATIVE, etc.), 
            # map it directly to its value in token_dict
            out.append(token_dict[token])
        else:
            # If the token is a number (e.g., 0333), break it down into individual digits
            for digit in token:
...
def scaled_dot_product_two_loop_single(
    ...
    # Get sequence length (K) and embedding dimension (M)
    K, M = query.shape
    ...
    scale = torch.sqrt(torch.tensor(M, dtype=torch.float32))

    # Iterate over each query in the sequence
    for i in range(K):
        scores = torch.zeros(K)
        
        # Compute scaled dot product for each key
        for j in range(K):
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        # Iterate over each query in the sequence
        for k in range(K):
            # Compute scaled dot product for all keys
            scores = torch.matmul(query[n, k], key[n].transpose(0, 1)) / scale
            
            # Apply softmax to get attention weights
            weights = torch.softmax(scores, dim=0)
            
            # Compute weighted sum of values
            out[n, k] = torch.matmul(weights, value[n])
...
def scaled_dot_product_no_loop_batch(
    ...
    weights = torch.bmm(query, key.transpose(1, 2)) / scale
    ...
    # Compute weighted sum of values
    y = torch.matmul(weights_softmax, value)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # Initialize weights for each linear layer using uniform distribution
    ...
    def forward(
        ...
        # Transform the input tensors using the initialized linear layers
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # Initialize a list of SingleHeadAttention layers
        self.attention_heads = nn.ModuleList([
            SelfAttention(dim_in, dim_out, dim_out)
            for _ in range(num_heads)
        ])

        # Initialize the linear layer to map the concatenated output back to dim_in
        self.linear_output = nn.Linear(num_heads * dim_out, dim_in)

        # Initialize the weights of the output linear layer
        c = math.sqrt(6 / (num_heads * dim_out + dim_in))
        self.linear_output.weight.data.uniform_(-c, c)
    ...
    def forward(
        ...
        # Compute the output of the MultiHeadAttention layer
        y = self.linear_output(multi_head_output)
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        self.gamma = nn.Parameter(torch.ones(emb_dim))  # Scale parameter
        self.beta = nn.Parameter(torch.zeros(emb_dim))  # Shift parameter
    ...
    def forward(self, x: Tensor):
        ...
        # Compute the mean across the embedding dimension (last dim)
        mean = x.mean(dim=-1, keepdim=True)

        # Compute the variance across the embedding dimension (last dim)
        squared_diffs = (x - mean).pow(2)  # Squared differences from the mean
        variance = squared_diffs.mean(dim=-1, keepdim=True)  # Mean of the squared differences along M dimension
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # Initialize weights for both MLPs
        c = math.sqrt(6 / (inp_dim + hidden_dim_feedforward))
        self.linear1.weight.data.uniform_(-c, c)
        self.linear2.weight.data.uniform_(-c, c)
    ...
    def forward(self, x):
        ...
        # Pass input through ReLU activation
        y = self.relu(y)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out1_norm = self.layer_norm1(out1 + x)  # Add & Norm
        out2 = self.dropout(out1_norm)

        # FeedForward
        out3 = self.feedforward(out2)
        out3_norm = self.layer_norm2(out3 + out2)  # Add & Norm
...
def get_subsequent_mask(seq):
    ...
    # Get batch size (N) and sequence length (K)
    N, K = seq.shape
    # Create a mask for the subsequent positions
    subsequent_mask = torch.triu(torch.ones((K, K), dtype=torch.bool), diagonal=1)
    # Expand the mask to apply to all sequences in the batch
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # 1. Masked MultiHeadAttention (self-attention for the decoder)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # Create a 1D tensor with K elements [0/K, 1/K, ..., (K-1)/K]
    pos_encoding_1d = torch.arange(K, dtype=torch.float32) / K
    
    # Expand it to shape (K, M) by repeating the positional encoding M times
    pos_encoding = pos_encoding_1d.unsqueeze(1).repeat(1, M)
    
    # Add batch dimension to get shape (1, K, M)
    y = pos_encoding.unsqueeze(0)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Initialize the positional encoding tensor with zeros with shape (K, M)
    y = torch.zeros(K, M)

    # Iterate over each position in the sequence
    for k in range(K):
        # Iterate over each dimension in the embedding
        for m in range(M):
            # Compute the denominator for the positional encoding
            denominator = 10000.0 ** float((2 * m) // M)

            # Apply sine to even indices (2i)
            if 2*m < M:
              y[k, 2*m] = math.sin(k / denominator)

            # Apply cosine to odd indices (2i + 1)
            if 2*m+1 < M:
              y[k, 2*m+1] = math.cos(k / denominator)

    # Reshape the positional encoding tensor to (1, K, M)
    y = y.unsqueeze(0)  
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # Pass q_emb_inp through the encoder
        enc_out = self.encoder(q_emb_inp)

        # Create the mask for the decoder
        mask = get_subsequent_mask(ans_b[:, :-1])

        # Move the mask to the same device as a_emb_inp
        mask = mask.to(a_emb_inp.device)

        # Pass a_emb_inp, encoder output, and mask to the decoder
        dec_out = self.decoder(a_emb_inp, enc_out, mask)

        # Reshape the output to (N*O, M)
        N, O, M = dec_out.shape
